...
namespace Nucleus {
	...
	static GLenum ShaderTypeFromString(const std::string& type) {
		...
		NC_CORE_ASSERT(false, "Unkown shader type!")
		return 0;
	}
	...
	std::unordered_map<GLenum, std::string>  OpenGLShader::PreProcess(const std::string& source) {
		...
		while (pos != std::string::npos)
		{
			...
			NC_CORE_ASSERT(ShaderTypeFromString(type), "Invalid shader type specified");
	...
	void  OpenGLShader::Compile(std::unordered_map<GLenum, std::string>& shaderSources) {
		...
		for (auto& keyValue : shaderSources) {
			...
			if (isCompiled == GL_FALSE)
			{
				...
				NC_CORE_ASSERT(false, "Shader compilation failure!");
				break;
			}
		...
		if (isLinked == GL_FALSE)
		{
			...
			NC_CORE_ERROR("{0}", infoLog.data());
			NC_CORE_ASSERT(false, "Shader link failure!");
			return;
		}
